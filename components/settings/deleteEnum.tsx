...
const DeleteEnum = (props: ModalProps) => {
    const submitHandler = async (e: React.MouseEvent<HTMLElement>) => {
        e.preventDefault();
        try {
            const response = await axios.delete(`${SERVER_URL}/enum`, {
            ...
            if (response.data.deleted) {
                ...
                alert('There has been an issue deleting this Enum, please try again.');
            }
        } catch (err) {
            alert('There has been an issue deleting this Enum, please try again.');
        }
    ...
    return (
        <div className="h-full w-full rounded-lg relative border-4 border-blue-200">
            <h1 className="w-full h-10 flex flex-row justify-center items-center font-bold bg-blue-200">Delete Enum</h1>
            <form className="flex flex-col justify-center items-center px-4 pt-2 overflow-y-auto h-[calc(100%-104px)]">
                <div className="px-10 font-semibold text-center mb-10">Name: {props.payload.name}</div>
                <div className="px-10 font-semibold text-center mb-10">You are about to delete this Enum, please click delete to confirm.</div>
