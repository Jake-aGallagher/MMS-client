...
const PropertyView = () => {
    ...
    const details = propertyDetails.map((property) => (
        <div className="pl-10 w-4/5 max-w-lg" key={property.id}>
    ...
    return (
        <>
            {viewModal ? (
                <ModalBase modalType={modalType} payload={parseInt(params.asPath.split('/')[2])} closeModal={() => [setViewModal(false), getPropertyHandler()]} />
            ) : null}
            {loading ? (
                <Loading />
            ) : noData ? (
                ...
                <div>
                    ...
                    <div>
                        ...
                        {/* <table>
                                <thead>
                                    <tr>
                                        <th className="border-2 border-solid border-slate-50 px-2">Username</th>
                                        <th className="border-2 border-solid border-slate-50 px-2">Name</th>
                                        <th className="border-2 border-solid border-slate-50 px-2">Authority</th>
                                    </tr>
                                </thead>
                                <tbody>{users}</tbody>
                            </table> */}
